...
export default defineConfig({
  ...
  rules: [
    ...
    [
      /^container-(medium|large|full|small)$/,
      ([, size]) => {
        const containerSize = {
          small: '56rem',
          medium: '64rem',
        ...
        return {
          'max-inline-size': maxInlineSizeFn(
            size as keyof typeof containerSize,
          ),
        }
      },
      { autocomplete: 'container-(medium|large|full|small)' },
    ],
  ],
  ...
  shortcuts: [
    ...
    {
      container:
        '[&>*]:md:max-w-2xl [&>*]:max-w-[calc(100%-clamp(1.5rem,1.4348rem+0.3261dvw,1.6875rem)*2)] [&>*]:mx-auto',
    },
