...
export default component$(() => {
	...
	const showValue = useComputed$(() => items.find(({ id }) => id === phone__or__email.value)?.value)
	...
	return (
		<div class={s.wrapper}>
			{items.map((item, index) => (
				<label class={s.eachRadio} key={item.id + index} onMouseEnter$={() => handleModeChange(item.id)}>
					<input
						type='radio'
						name='contact'
						value={item.id}
