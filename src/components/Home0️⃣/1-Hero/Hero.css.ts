...
export const name = styleVariants(
	{
		...
		lastName: 'light-dark(oklch(62.17% 0.1535 49.76),oklch(76.47% 0.1535 49.76))',
	},
...
export const image = style({
	...
	boxShadow: boxShadowGenerator(
		[T.color.green, T.color.azure, T.color.red, T.color.green, T.color.azure, T.color.red],
		3.5,
	),
})
...
const bubleColors: Record<string, Buble> = {
	blue: {
		colors: ['oklch(58.01% 0.1316 249.88)', 'oklch(65.73% 0.1316 249.88)', 'oklch(71.96% 0.1316 249.88)'],
		position: {
			left: 50,
			top: 100,
		},
...
export const bubble = styleVariants(bubleColors, ({ colors, position: { left, top } }) => [
	bubbleBase,
	{
		display: 'none',
		'@media': {
			[T.media.md]: {
				...
				left,
				top,
				boxShadow: colors.map(color => `0 0 min(30dvw,100vw) 150px ${color}`).join(', '),
			},
		},
	},
])
