...
const ZeroData: TarifProps[] = [
	{
		title: 'Abonnement',
		description: 'Vous payez une seule et unique fois ! Aucun coût récurrent ne vous sera demandé.',
		icon: '🆓',
	},
	{
		title: 'Hébergement',
...
export const Zero = component$(() => {
	...
	const zeroEffect = useSignal<boolean>(true)
	...
	const handleScroll = $(() => {
		...
		if (newActiveIndex !== activeIndex.value) {
			activeIndex.value = Math.max(0, Math.min(newActiveIndex, ZeroData.length - 1))
		}
	})
	...
	return (
		<section class={s.wrapper}>
			<h3 class={[s.zeroBase, zeroEffect.value ? s.zeroActive : s.zeroEffect]}>0€</h3>
			<ul class={s.ul}>
				{ZeroData.map((item, i) => (
					<li
						key={`${item.title} ${i}`}
						...
						style={{
							backgroundColor: activeIndex.value === i ? '#e0e0e0' : 'transparent',
						}}
					>
						{item.title}
			...
			<div class={s.content}>
				<button type='button' class={s.contentArrow} onClick$={() => move('LEFT')} disabled={!buttonState.prev.value}>
					←
				</button>
				...
				<button type='button' class={s.contentArrow} onClick$={() => move('RIGHT')} disabled={!buttonState.next.value}>
					→
				</button>
			</div>
		</section>
