...
interface DentalNavaProps extends aItem {
	isMobile?: boolean;
	onClick$?: QRL<() => void>;
}
const DentalNava = component$<DentalNavaProps>((props) => {
	return (
		<li>
			<a
				href={props.a}
...
export const Navbar = component$(() => {
	...
	useTask$(({ track }) => {
		...
		if (!isMobileMenuOpen.value) {
			for (const key of Object.keys(mobileSubmenuStates)) {
			...
			<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				<div class="flex h-16 justify-between">
					...
					<ul class="hidden items-center space-x-1 md:flex">
						{menu.map((item, index) =>
							item.type === "a" ? (
								<DentalNava key={`${item.name}-${index}`} {...item} />
							) : (
								<DesktopSubmenuItem
									key={`${item.name}-${index}`}
									{...(item as SubmenuItem)}
